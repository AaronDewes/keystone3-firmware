...
void GuiFirmwareUpdateSha256Percent(uint8_t percent)
{
    ...
    if (lv_obj_is_valid(g_calCheckSumLabel)) {
        lv_label_set_text_fmt(g_calCheckSumLabel, "#F5870A Show Checksum(%d%%)#", percent);
        if (percent == 100) {
...
static void GuiCreateSdCardnstructionTile(lv_obj_t *parent)
{
    ...
    if (FatfsFileExist(OTA_FILE_PATH)) {
        btn = GuiCreateBtn(parent, _("Verify Firmware"));
        lv_obj_set_style_bg_opa(btn, LV_OPA_0, LV_PART_MAIN);
...
static void GuiFirmwareUpdateViewSha256(uint8_t percent)
{
    ...
    if (percent == 100) {
        ...
        label = GuiCreateNoticeLabel(g_noticeHintBox, _("CheckSum\n\n"));
        lv_label_set_recolor(label, true);
        lv_obj_align(label, LV_ALIGN_BOTTOM_LEFT, 36, -130);

        lv_obj_t *desc = GuiCreateIllustrateLabel(g_noticeHintBox, "This is an optional feature to further enhance security. Compare the following checksum with the checksum of your download package on the official website, ensure that they are consistent.");
